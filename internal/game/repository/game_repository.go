...
import (
	...
	profileRepo "github.com/OmkarGeedh/GamifiedQuizApp_digitalHQ/internal/profile/repository"
	"gorm.io/gorm"
)
...
func FinalizeSession(ctx context.Context, session *models.GameSession, coins, xp, gems int) error {
	return getDB().WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Model(&profileModels.Profile{}).
		...
		_ = profileRepo.RecordDailyActivity(ctx, session.ClientID, todayStr)
