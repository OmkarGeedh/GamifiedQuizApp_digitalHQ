...
import (
	...
	"github.com/OmkarGeedh/GamifiedQuizApp_digitalHQ/internal/game/services"
...
var Manager *gameWs.SessionManager
...
func GetGameHistoryHandler(c *gin.Context) {
	...
	sessions, total, err := repository.GetClientGameHistory(c.Request.Context(), clientID, limit, offset)
	if err != nil {
...
func WebSocketGameHandler(c *gin.Context) {
	...
	session, err := repository.GetSessionByID(c.Request.Context(), sessionID)
	if err != nil {
	...
	if err != nil || len(questions) == 0 {
		...
		dbQuestions, err := repository.GetQuestionsByTopic(c.Request.Context(), session.TopicID, session.TotalQuestions)
	...
	go func() {
		...
		for {
			...
			switch msg.Type {
			case gameWs.MsgTypeSubmitAnswer:
				gs.PushEvent(gameWs.MsgTypeSubmitAnswer, msg.Data)
			default:
