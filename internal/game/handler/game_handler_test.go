...
func TestE2E_MCQ_REST_Workflow(t *testing.T) {
	...
	_ = config.DB.Model(&gameModels.GameSession{}).
		Where("client_id = ? AND status = ?", 1, gameModels.SessionStatusInProgress).
		Update("status", gameModels.SessionStatusAbandoned).Error
	...
	t.Run("Complete session awards wallet ledger and profile updates", func(t *testing.T) {
		...
		config.DB.Model(&gameModels.WalletLedger{}).
		...
		var dbSession gameModels.GameSession
		config.DB.Where("id = ?", activeSessionID).First(&dbSession)
		if dbSession.Status != gameModels.SessionStatusFinished {
	...
	t.Run("Fetch player quiz history", func(t *testing.T) {
		...
		var resp struct {
			...
			Data    struct {
				...
				Sessions []gameModels.GameSession `json:"sessions"`
			} `json:"data"`
		}
		if err := json.Unmarshal(w.Body.Bytes(), &resp); err != nil {
			t.Fatalf("Failed to decode response: %v", err)
	...
	t.Run("Abandon active session", func(t *testing.T) {
		...
		if dbSession.Status != gameModels.SessionStatusAbandoned {
...
func TestE2E_WebSocket_LiveGame(t *testing.T) {
	...
	submitMsg := gameWs.InboundMessage{
		Type: gameWs.MsgTypeSubmitAnswer,
	}
	submitData, _ := json.Marshal(gameWs.SubmitAnswerData{
