...
func (gs *GameSession) handleAnswer(ctx context.Context, rawData json.RawMessage) {
	...
	// Record in DB
	_ = repository.RecordAnswer(ctx, &models.UserQuestionHistory{
...
func (gs *GameSession) advanceQuestion(ctx context.Context) {
	...
	// Persist progress
	_ = repository.UpdateSession(ctx, gs.Session)
...
func (gs *GameSession) handleGameEnd(ctx context.Context) {
	...
	profile, err := profileServices.GetOrCreateProfile(ctx, gs.ClientID)
	newLevel := 1
	didLevelUp := false
	if err == nil {
	...
	// Atomic finalization
	_ = repository.FinalizeSession(ctx, gs.Session, coins, xp, gems)

	if didLevelUp && profile != nil {
		_ = repository.UpdateProfileLevel(ctx, gs.ClientID, newLevel)
	}
